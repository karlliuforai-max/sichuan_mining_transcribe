...
- 关键数据:价格、品位、数量、金额、比例、日期 —— 必须逐字保留原文数字,并在每个数字后附上它所在的原文短句,格式如:`品位 0.8(原文:"实际平均品位只有零点八几")`
- 达成的共识或决定
...
1. 用 Markdown 输出,结构为:
   ...
   ## 会议信息(日期填上方会议日期;参会方根据要点中的称呼归纳,认不出就写"待补充",绝不要列"发言人N")
   ## 核心结论与共识
...
2. 所有数字、金额必须来自要点原文,不得推算或编造;"原文摘录"列填该数据对应的原话片段。
3. 不要给每一项都加"待核实"。只对以下两类标注"(待核实)":① 上方存疑列表中提到的内容;② 你确实判断转写可能有歧义的数字或名称。其余有把握的内容直接干净呈现。
4. 语言简洁正式,可直接发给参会方。
